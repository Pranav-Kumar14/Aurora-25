...
const Carousel = () => {
  ...
  useEffect(() => {
    ...
    }, 2000); // Speed up the carousel by reducing interval to 2 seconds
            ...
            <div
              className={`
                ...
                ${index === Math.floor(visibleCount / 2)
                  ? 'scale-110 shadow-lg' // Active item with shadow and scale
                  : 'scale-90 opacity-50'}
                transition-transform duration-700 ease-in-out
              `}
              style={{
                boxShadow: index === Math.floor(visibleCount / 2) 
                  ? '0 0 15px 4px rgba(0, 123, 255, 0.7), 0 0 15px 4px rgba(0, 123, 255, 0.5)' // Gradient-like glow effect
                  : 'none',
                transition: 'box-shadow 0.7s ease, transform 0.7s ease' // Smooth shadow and scale transitions
              }}
            >
              <img
                src={item.src}
